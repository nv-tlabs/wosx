...
try:
    ...
    try:
        ...
        # Directory containing the packaged slang shaders, laid out with the
        # same relative structure as a source checkout. Pass this as the
        # wosx_directory_path argument of GPUTaskHandle when using an installed
        # package; when running from a source checkout, pass the repository root
        # instead (as the demos do), so shader edits take effect without reinstalling.
        import os as _os
        slang_directory_path = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "slang")
        ...
        # NOTE: GPUPointEstimator is intentionally not dispatched: it binds no
        # constructor, since instances are obtained from a GPUTaskHandle; the
        # suffixed classes (e.g. GPUPointEstimator_float_2d) remain available
        # for type checks
        for _name in [
            "GPUSampleStatistics",
        ...
        Samplers.GPUUniformLineSegmentBoundarySampler = _fixed_dim_factory(
            Samplers, "GPUUniformLineSegmentBoundarySampler", 2)
        Samplers.GPUUniformTriangleBoundarySampler = _fixed_dim_factory(
            Samplers, "GPUUniformTriangleBoundarySampler", 3)
